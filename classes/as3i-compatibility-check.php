...
if ( ! class_exists( 'as3i_Compatibility_Check' ) ) {
	...
	class as3i_Compatibility_Check {
		...
		public function get_error_msg() {
			...
			$deactivate_link = sprintf( '<a style="text-decoration:none;" href="%s">%s</a>', $deactivate_url, __( 'deactivate', 'aws-s3-integration' ) );
			$hide_notice_msg = '<br><em>' . sprintf( __( 'You can %s the %s plugin to get rid of this notice.', 'aws-s3-integration' ), $deactivate_link, $this->plugin_name ) . '</em>';
			...
			if ( ! $this->is_parent_plugin_enabled() ) {
				$msg = sprintf( __( '%s has been disabled as it requires the %s plugin.', 'aws-s3-integration' ), $this->plugin_name, $parent_plugin_link_html );
				...
				if ( file_exists( WP_PLUGIN_DIR . '/' . $parent_basename ) ) {
					if ( isset( $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['compatible'] ) && ! $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['compatible'] ) {
						$msg = rtrim( $msg, '.' ) . ', ' . __( 'which is currently disabled.', 'aws-s3-integration' );
					} else {
						$msg          .= ' ' . __( 'It appears to be installed already.', 'aws-s3-integration' );
					...
					$msg .= ' ' . sprintf( __( '<a href="%s">Install</a> and activate it.', 'aws-s3-integration' ), $install_url );
				}
			...
			if ( ! version_compare( $current_parent_plugin_version, $this->parent_plugin_required_version, '>=' ) ) {
				$msg = sprintf( __( '%s has been disabled as it requires version %s or later of the %s plugin.', 'aws-s3-integration' ), $this->plugin_name, $this->parent_plugin_required_version, $parent_plugin_link_html );
				...
				$msg .= ' <a style="font-weight:bold;text-decoration:none;white-space:nowrap;" href="' . $update_url . '">' . __( 'Update to the latest version', 'aws-s3-integration' ) . '</a>';
				$msg .= $hide_notice_msg;
			...
			if ( ! isset( $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['supported_addon_versions'][ $this->plugin_slug ] ) ) {
				$msg = sprintf( __( '%1$s has been disabled because it is not a supported addon of the %2$s plugin.', 'aws-s3-integration' ), $this->plugin_name, $this->get_parent_plugin_name() );
			...
			if ( ! version_compare( $this_plugin_version, $this_plugin_version_required, '>=' ) ) {
				$msg = sprintf( __( '%1$s has been disabled because it will not work with the version of the %2$s plugin installed. %1$s %3$s or later is required.', 'aws-s3-integration' ), $this->plugin_name, $this->get_parent_plugin_name(), $this_plugin_version_required );
		...
		function get_admin_notice() {
			...
			if ( $this->deactivate_if_not_compatible ) {
				$deactivated_msg = sprintf( __( 'The %s plugin has been deactivated.', 'aws-s3-integration' ), $this->plugin_name );
		...
		public static function deactivate_other_instances( $plugin ) {
			if ( ! in_array( basename( $plugin ), array( 'aws-s3-integration-pro.php', 'wordpress-s3.php' ) ) ) {
				return false;
			}
			...
			if ( basename( $plugin ) === $plugin_to_deactivate ) {
				$plugin_to_deactivate             = 'aws-s3-integration-pro.php';
		...
		public function get_sdk_requirements_errors() {
			...
			if ( version_compare( PHP_VERSION, '5.5', '<' ) ) {
				$errors[] = __( 'a PHP version less than 5.5', 'aws-s3-integration' );
			}

			if ( ! class_exists( 'SimpleXMLElement' ) ) {
				$errors[] = __( 'no SimpleXML PHP module', 'aws-s3-integration' );
			}

			if ( ! class_exists( 'XMLWriter' ) ) {
				$errors[] = __( 'no XMLWriter PHP module', 'aws-s3-integration' );
			}

			if ( ! function_exists( 'curl_version' ) ) {
				$errors[] = __( 'no PHP cURL library activated', 'aws-s3-integration' );

				return $errors;
			}

			if ( ! ( $curl = curl_version() ) || empty( $curl['version'] ) || empty( $curl['features'] ) || version_compare( $curl['version'], '7.16.2', '<' ) ) {
				$errors[] = __( 'a cURL version less than 7.16.2', 'aws-s3-integration' );
			}
			...
			if ( ! empty( $curl['features'] ) ) {
				...
				if ( $curl_errors ) {
					$errors[] = __( 'cURL compiled without', 'aws-s3-integration' ) . ' ' . implode( ' or ', $curl_errors ); // xss ok
				}
			}

			if ( ! function_exists( 'curl_multi_exec' ) ) {
				$errors[] = __( 'the function curl_multi_exec disabled', 'aws-s3-integration' );
			}
		...
		public function get_sdk_error_msg() {
			...
			$msg = __( 'The official Amazon&nbsp;Web&nbsp;Services SDK requires PHP 5.5+ with SimpleXML and XMLWriter modules, and cURL 7.16.2+ compiled with OpenSSL and zlib. Your server currently has', 'aws-s3-integration' );
