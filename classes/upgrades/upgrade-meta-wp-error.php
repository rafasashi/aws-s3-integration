...
class Upgrade_Meta_WP_Error extends Upgrade {
	...
	protected function get_running_update_text() {
		return __( 'and rebuilding the metadata for attachments that may have been corrupted.', 'aws-s3-integration' );
	}
	...
	protected function upgrade_item( $attachment ) {
		...
		if ( ! file_exists( $file ) ) {
			...
			try {
				...
				AS3CF_Error::log( sprintf( __( 'There was an error attempting to download the file %s from the bucket: %s', 'aws-s3-integration' ), $provider_object['key'], $e->getMessage() ) );
