...
abstract class Upgrade {
	...
	public function maybe_display_notices() {
		...
		switch ( $this->get_upgrade_status() ) {
			case self::STATUS_RUNNING:
				...
				$action_text = __( 'Pause Update', 'aws-s3-integration' );
				$action_url  = $this->as3cf->get_plugin_page_url( array(
					'action' => 'pause_update',
					'update' => $this->upgrade_name,
				), 'self' );
			...
			case self::STATUS_PAUSED:
				...
				$action_text = __( 'Restart Update', 'aws-s3-integration' );
				break;
			case self::STATUS_ERROR:
				$msg         = $this->get_error_message();
				$action_text = __( 'Try To Run It Again', 'aws-s3-integration' );
				$msg_type    = 'error';
				break;
			default:
				return;
	...
	protected function get_paused_message() {
		return sprintf( __( '<strong>%1$s Update Paused%2$s</strong> &mdash; Updating Media Library %3$s has been paused.', 'aws-s3-integration' ),
			ucwords( $this->upgrade_type ),
			$this->get_progress_text(),
			$this->upgrade_type
		);
	...
	protected function get_error_message() {
		return sprintf( __( '<strong>Error Updating %1$s</strong> &mdash; We ran into some errors attempting to update the %2$s for all your Media Library items that have been offloaded. Please check your error log for details. (#%3$d)', 'aws-s3-integration' ),
			ucwords( $this->upgrade_type ),
			$this->upgrade_type,
			$this->upgrade_id
		);
	...
	protected function get_progress_text() {
		...
		return sprintf( __( ' (%s%% Complete)', 'aws-s3-integration' ), $progress );
	}
	...
	public function cron_schedules( $schedules ) {
		...
		$schedules[ $this->cron_schedule_key ] = array(
			...
			'display'  => sprintf( __( 'Every %d Minutes', 'aws-s3-integration' ), $this->cron_interval_in_minutes ),
		);
