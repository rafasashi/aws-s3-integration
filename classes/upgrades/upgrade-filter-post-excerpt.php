...
class Upgrade_Filter_Post_Excerpt extends Upgrade_Filter_Post {
	...
	protected function get_running_update_text() {
		return __( 'and ensuring that only the local URL exists in post excerpts.', 'aws-s3-integration' );
	}
	...
	protected function get_running_message() {
		return sprintf( __( '<strong>Running Excerpts Upgrade%1$s</strong><br>A find &amp; replace is running in the background to update URLs in your post excerpts. %2$s', 'aws-s3-integration' ), $this->get_progress_text(), $this->get_generic_message() );
	}
}