...
abstract class Upgrade_Filter_Post extends Upgrade {
	...
	protected function get_paused_message() {
		return sprintf( __( '<strong>Paused Upgrade</strong><br>The find &amp; replace to update URLs has been paused. %s', 'aws-s3-integration' ), $this->get_generic_message() );
	}
	...
	protected function get_generic_message() {
		$link_text = __( 'See our documentation', 'aws-s3-integration' );
		...
		return sprintf( __( '%s for details on why we&#8217;re doing this, why it runs slowly, and how to make it run faster.', 'aws-s3-integration' ), $link );
	}
