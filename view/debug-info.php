<div class="debug support-section">
	<h3><?php _e( 'Diagnostic Info', 'aws-s3-integration' ); ?></h3>
	<textarea class="debug-log-textarea" autocomplete="off" readonly></textarea>
	<?php
	$args = array(
		'nonce'              => wp_create_nonce( 'as3i-download-log' ),
		'as3i-download-log' => '1',
		'hash'               => 'support',
	);

	$url = $this->get_plugin_page_url( $args, 'network', false );
	?>
	<a href="<?php echo esc_url( $url ); ?>" class="button"><?php _ex( 'Download', 'Download to your computer', 'aws-s3-integration' ); ?></a>
</div>