...
<div id="tab-media" data-prefix="as3cf" class="as3cf-tab as3cf-content<?php echo $storage_classes; // xss ok ?>">
			...
			<table class="form-table as3cf-media-settings">
				...
				<tr class="configure-url">
					<td colspan="2">
						...
						$seo_friendly_url_msg   = apply_filters( 'as3cf_seo_friendly_url_notice', sprintf( __( 'Yikes! That\'s not a very SEO-friendly URL. We strongly recommend you configure a CDN to point at your bucket and configure a subdomain of %1$s to point at your CDN. %2$s', 'aws-s3-integration' ), AS3CF_Utils::current_base_domain(), $seo_friendly_url_link ) );
				...
				<tr class="as3cf-setting-title">
					<td colspan="2"><h3><?php _e( 'Storage', 'aws-s3-integration' ); ?></h3></td>
				</tr>
				...
				<!-- URL Rewriting -->
				<tr class="as3cf-setting-title">
					<td colspan="2"><h3><?php _e( 'URL Rewriting', 'aws-s3-integration' ); ?></h3></td>
				</tr>
				...
				<tr class="advanced-options as3cf-setting-title">
					<td colspan="2"><h3><?php _e( 'Advanced Options', 'aws-s3-integration' ); ?></h3></td>
				</tr>
				...
				<tr class="advanced-options <?php echo $args['tr_class']; ?>">
					...
					<td>
						...
						<p><?php _e( 'Once a file has been copied to the bucket, remove it from the local server.', 'aws-s3-integration' ); ?>
							<?php echo $this->settings_more_info_link( 'remove-local-file', 'media+remove+files+from+server' ); ?>
						</p>
						<?php
						$lost_files_msg  = apply_filters( 'as3cf_lost_files_notice', __( '<strong>Broken URLs</strong> &mdash; There will be broken URLs for files that don\'t exist locally. You can fix this by enabling <strong>Rewrite Media URLs</strong> to use the offloaded media.', 'aws-s3-integration' ) );
						...
						$remove_local_msg  = sprintf( __( '<strong>Warning</strong> &mdash; Some plugins depend on the file being present on the local server and may not work when the file is removed. %s', 'aws-s3-integration' ), $remove_local_link );
						$remove_local_msg  .= "<br/><br />";
						$remove_local_msg  .= __( 'If you have a backup system in place (as you should) that backs up your site files, media, and database, your media will no longer be backed up as it will no longer be present on the filesystem.', 'aws-s3-integration' );
	...
	if ( $this->get_provider()->needs_access_keys() ) {
		...
		<p class="as3cf-need-help">
			...
			<?php printf( __( 'Need help getting your Access Keys? <a href="%s">Check out the Quick Start Guide &rarr;</a>', 'aws-s3-integration' ), $this->dbrains_url( '/wp-offload-media/doc/quick-start-guide/', array(
