...
<div class="as3cf-content as3cf-provider-select">
	...
	if ( ! empty( $_GET['action'] ) && 'change-provider' === $_GET['action'] && $this->get_setting( 'bucket' ) ) {
		echo '<a href="' . $this->get_plugin_page_url() . '">' . __( '&laquo;&nbsp;Back', 'aws-s3-integration' ) . '</a>';
	}
	?>
	<h3><?php _e( 'Storage Provider', 'aws-s3-integration' ) ?></h3>
			...
			<tr class="as3cf-provider-title as3cf-provider-<?php echo $provider_key; ?><?php echo $provider_selected_class; ?>">
				...
				<td>
					...
					if ( $provider_selected && $provider_defined ) {
						echo '<span class="as3cf-defined-in-config">' . __( 'defined in wp-config.php', 'aws-s3-integration' ) . '</span>';
					}
					?>
					...
					<table>
						<?php if ( ! $provider_selected && ! empty( $media_offloaded_string ) ) { ?>
							<tr>
								<td colspan="2">
									<?php
									$message_string = sprintf( __( '<strong>Warning:</strong> You have %s offloaded Media Library items, you should remove them from the bucket before changing storage provider.', 'aws-s3-integration' ), $media_offloaded_string );
						...
						if ( $provider_class::use_access_keys_allowed() ) {
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-define"><?php _e( 'Define access keys in wp-config.php', 'aws-s3-integration' ) ?></label>
								</td>
							</tr>
								...
								<td>
									...
									if ( $any_access_key_constant_defined ) {
										if ( count( $defined_constants ) > 1 ) {
											$remove_defines_msg = _x( 'You\'ve defined your access keys in your wp-config.php. To select a different option here, simply comment out or remove the \'%1$s\' defines in your wp-config.php.', 'Access Keys defined in multiple defines.', 'aws-s3-integration' );
										} else {
											$remove_defines_msg = _x( 'You\'ve defined your access keys in your wp-config.php. To select a different option here, simply comment out or remove the \'%1$s\' define in your wp-config.php.', 'Access Keys defined in single define.', 'aws-s3-integration' );
										}
										$multiple_defined_keys_glue = _x( ' & ', 'joins multiple define keys in notice', 'aws-s3-integration' );
										...
										if ( $provider_selected && ! $provider->are_access_keys_set() ) {
											...
											<div class="notice-error notice">
												<p>
													<?php _e( 'Please check your wp-config.php file as it looks like one of your access key defines is missing or incorrect.', 'aws-s3-integration' ) ?>
												</p>
											</div>
											<?php
										}
									} else {
										_e( 'Copy the following snippet <strong>near the top</strong> of your wp-config.php and replace the stars with the keys.', 'aws-s3-integration' );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-access-keys' );
										?>
...
define( '<?php echo $this::preferred_settings_constant(); ?>', serialize( array(
    ...
    'secret-access-key' => '**************************************',
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-define"><?php _e( 'Define key file path in wp-config.php', 'aws-s3-integration' ) ?></label>
								</td>
							</tr>
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="define"<?php echo 'define' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								<td></td>
								<td>
									<?php
									if ( $any_access_key_constant_defined ) {
										$remove_defines_msg         = _x( 'You\'ve defined your key file path in your wp-config.php. To select a different option here, simply comment out or remove the \'%1$s\' define in your wp-config.php.', 'Key file path defined in single define.', 'aws-s3-integration' );
										$multiple_defined_keys_glue = _x( ' & ', 'joins multiple define keys in notice', 'aws-s3-integration' );
										$defined_constants_str      = join( $multiple_defined_keys_glue, $defined_constants );
										printf( $remove_defines_msg, $defined_constants_str );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-key-file' );
									} else {
										_e( 'Copy the following snippet <strong>near the top</strong> of your wp-config.php and replace "<strong>/path/to/key/file.json</strong>".', 'aws-s3-integration' );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-key-file' );
										?>
...
define( '<?php echo $this::preferred_settings_constant(); ?>', serialize( array(
    ...
    'key-file-path' => '/path/to/key/file.json',
						...
						if ( $provider_class::use_server_roles_allowed() ) {
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-server-role"><?php printf( __( 'My server is on %s and I\'d like to use IAM Roles', 'aws-s3-integration' ), $provider_class::get_provider_name() ) ?></label>
								</td>
							</tr>
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="server-role"<?php echo 'server-role' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								...
								<td>
									<?php if ( $any_use_server_roles_constant_defined ) {
										$remove_defines_msg         = _x( 'You\'ve defined use of server roles in your wp-config.php. To select a different option here, simply comment out or remove the \'%1$s\' define in your wp-config.php.', 'Use Server Roles defined in single define.', 'aws-s3-integration' );
										$multiple_defined_keys_glue = _x( ' & ', 'joins multiple define keys in notice', 'aws-s3-integration' );
										$defined_constants_str      = join( $multiple_defined_keys_glue, $defined_use_server_roles_constants );
										printf( $remove_defines_msg, $defined_constants_str );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-access-keys' );
									} else {
										printf( __( 'If you host your WordPress site on %s, choose this option and make use of IAM Roles.', 'aws-s3-integration' ), $provider_class::get_provider_name() );
						...
						if ( $provider_class::use_access_keys_allowed() ) {
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-db"><?php _e( 'I understand the risks but I\'d like to store access keys in the database anyway (not recommended)', 'aws-s3-integration' ) ?></label>
								</td>
							</tr>
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="db"<?php echo 'db' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								...
								<td>
									<?php
									_e( 'Storing your access keys in the database is less secure than the options above, but if you\'re ok with that, go ahead and enter your keys in the form below.', 'aws-s3-integration' );
									echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-access-keys' );
									?>
									<table class="form-table as3cf-access-keys">
										<tr valign="top">
											<th scope="row"><?php _e( 'Access Key ID', 'aws-s3-integration' ) ?></th>
											<td>
										...
										<tr valign="top">
											<th scope="row"><?php _e( 'Secret Access Key', 'aws-s3-integration' ) ?></th>
											<td>
												<div class="as3cf-field-wrap">
													<input
														type="text"
														name="secret-access-key"
														value="<?php echo $provider_selected && $provider->get_secret_access_key() ? _x( '-- not shown --', 'placeholder for hidden secret access key, 39 char max', 'aws-s3-integration' ) : '' ?>"
														autocomplete="off"
														<?php echo $provider_selected ? '' : ' disabled="disabled"'; ?>
													>
												</div>
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-db"><?php _e( 'I understand the risks but I\'d like to store the key file\'s contents in the database anyway (not recommended)', 'aws-s3-integration' ) ?></label>
								</td>
							</tr>
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="db"<?php echo 'db' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								<td></td>
								<td>
									<?php
									_e( 'Storing your key file\'s contents in the database is less secure than the options above, but if you\'re ok with that, go ahead and enter your key file\'s JSON data in the field below.', 'aws-s3-integration' );
									echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-key-file' );
									?>
	...
	<p>
		<button type="submit" class="button button-primary"><?php echo empty( $this->get_defined_setting( 'bucket', false ) ) ? __( 'Next', 'aws-s3-integration' ) : __( 'Save Changes', 'aws-s3-integration' ); ?></button>
	</p>
</div>
